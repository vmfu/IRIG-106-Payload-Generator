		...
		function drawWaterfall() {
			...
			// Вычисляем длину кадра в байтах
			// Если длина данных не делится нацело, Math.ceil округлит вверх (паддинг последнего байта)
			const bytesPerFrame = Math.ceil(data.length / numFrames);
			
			// Устанавливаем РЕАЛЬНЫЙ размер холста (1 пиксель = 1 байт)
			canvas.width = bytesPerFrame;
			canvas.height = numFrames;

			// Создаем буфер изображения для прямого доступа к пикселям (быстрее, чем fillRect)
			const imgData = ctx.createImageData(bytesPerFrame, numFrames);
			const pixels = imgData.data;

			for (let y = 0; y < numFrames; y++) {
				for (let x = 0; x < bytesPerFrame; x++) {
					// Индекс байта в бинарном массиве
					const byteIndex = y * bytesPerFrame + x;
					
					let val = 0;
					// Если мы не вышли за пределы массива данных
					if (byteIndex < data.length) {
						val = data[byteIndex];
					} else {
						// Паддинг нулями (если данных не хватило на полный байт/кадр)
						val = 0;
					}
					
					// Индекс пикселя в Canvas (RGBA)
					const pIndex = (y * bytesPerFrame + x) * 4;
					
					// Отрисовка: используем значение байта как яркость (Grayscale)
					// Можно сделать "Матрицу" (зеленый): R=0, G=val, B=0
					pixels[pIndex] = val;     // Red
					pixels[pIndex + 1] = val; // Green
					pixels[pIndex + 2] = val; // Blue
					pixels[pIndex + 3] = 255; // Alpha (Непрозрачность)
				}
			}
